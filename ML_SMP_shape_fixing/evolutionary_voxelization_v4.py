...
CHECK_POINT = "checkpoint_objet_sinusoid_100_by_5_9_6_deflection.pkl"
    ...
    if failed:
        ...
        for key in columns.keys():
            if 'y_displacement' in key:
                ...
                if evaluatebest:
                    analytic_uy = target_shape_function(sorted_xs)

                    title = key.split('_')[-1]

                    plt.figure(1)
                    plt.plot(sorted_xs, analytic_uy,
                             color='k', label='Desired Shape')
                ...
                error = target_shape_error(temp_xs, uy, stats)
                errors.append(error)
...
def target_shape_function(xs):
    analytic_uy = -9.6 * (np.ones(len(xs)) - np.cos(np.pi*xs/80.0))
...
def target_shape_error(xs, uy, stats=False):
    ...
    analytic_uy = target_shape_function(xs)
...
def evaluate_best(checkpoint):
    ...
    error1 = evaluate_design(halloffame[0], gui=True, evaluatebest=True)
